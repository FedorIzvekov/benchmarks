...
public class LinkedListIteration {
    ...
    public void for1(Data data, Blackhole blackhole) {
        for (int count = 0; count < data.linkedList.size(); count++) {

            workingLoad(data.linkedList.get(count), blackhole);
    ...
    public void forEach(Data data, Blackhole blackhole) {
        for (String element : data.linkedList) {
    ...
    public void while1(Data data, Blackhole blackhole) {
        ...
        while (count < data.linkedList.size()) {
    ...
    public void whileIterator(Data data, Blackhole blackhole) {
        Iterator<String> iterator = data.linkedList.iterator();
    ...
    public void stream(Data data, Blackhole blackhole) {
        data.linkedList.stream().forEach(element -> workingLoad(element, blackhole));
    }
    ...
    public static class Data {
        ...
        @Setup
        public void setup() {
            linkedList = Stream.generate(() -> generate(stringLength))
                    .limit(capacity)
